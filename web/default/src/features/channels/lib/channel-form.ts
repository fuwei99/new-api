...
export const channelFormSchema = z
  .object({
    ...
    preserve_thinking_suffix: z.boolean().optional(), // Anthropic: preserve thinking suffix
...
export function transformChannelToFormDefaults(
  ...
  let preserveThinkingSuffix = false
  ...
  if (channel.settings) {
    try {
      ...
      preserveThinkingSuffix = parsed.preserve_thinking_suffix === true
  ...
  return {
    ...
    preserve_thinking_suffix: preserveThinkingSuffix,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Anthropic (type 14): claude_beta_query, allow_inference_geo, allow_speed, preserve_thinking_suffix
  if (formData.type === 14) {
    settingsObj.allow_inference_geo = formData.allow_inference_geo === true
    settingsObj.allow_speed = formData.allow_speed === true
    settingsObj.claude_beta_query = formData.claude_beta_query === true
    settingsObj.preserve_thinking_suffix =
      formData.preserve_thinking_suffix === true
  } else {
    ...
    if ('preserve_thinking_suffix' in settingsObj)
      delete settingsObj.preserve_thinking_suffix
  }
